...
def get_county_code_to_object_keys_dict(
    object_key_to_county_codes: Dict[str, List[str]]
) -> Dict[str, List[str]]:
...
def generate_density_map(
    county_map: CountyMap, county_code_to_object_keys_dict: Dict[str, List[str]]
):
    ...
    max_num_objects = max(
        [len(object_keys) for object_keys in county_code_to_object_keys_dict.values()]
    )
